...
export async function sendAbandonedCartEmail({
  ...
  if (!process.env.RESEND_API_KEY || process.env.RESEND_API_KEY === 'placeholder') {
    console.log('RESEND_API_KEY is missing, skipping abandoned cart email.');
    return { success: false, error: 'RESEND_API_KEY is missing' };
  }
...
<head>
    <meta charset="utf-8">
    <style>
        body { font-family: sans-serif; background-color: #050505; color: #FFFFFF; }
        .container { max-width: 600px; margin: 0 auto; background-color: #171717; padding: 40px; border-radius: 8px; }
        .button { display: inline-block; padding: 12px 24px; background: #3B82F6; color: #FFFFFF; text-decoration: none; border-radius: 8px; font-weight: bold; }
    </style>
</head>
<body>
    <div class="container">
        <h1>Did you forget something?</h1>
        <p>We noticed you were looking at <strong>${productName}</strong> but didn't finish your purchase.</p>
        <p>The Forge is still hot. Your assets are waiting for you to claim them.</p>
    ...
    const { data, error } = await resend.emails.send({
      from,
      to,
      subject: `Don't miss out on ${productName} - EtherForge`,
      html,
    });
